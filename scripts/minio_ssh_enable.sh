...
echo "create ca-cert"
openssl req -new -x509 -nodes -days 365000 -key ca-key.pem -out ca-cert.pem -subj "/CN=localhost" -addext "subjectAltName = DNS:$SAN"

echo "view ca-cert"
openssl x509 -noout -text -in ca-cert.pem

echo "create server-key and csr (using $PASSWD as password)"
openssl req -new -subj "/CN=localhost" -addext "subjectAltName = DNS:$SAN" -passout pass:$PASSWD -newkey rsa:2048 -keyout server-key.pem -out server-req.pem
